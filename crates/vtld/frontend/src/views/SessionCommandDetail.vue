...
import { fetchSessionScsiLogEntry, type ScsiCommandDetail } from '../api'

const route = useRoute()

const tsih = computed(() => Number(route.params.tsih) || 0)
...
onMounted(async () => {
  const data = await fetchSessionScsiLogEntry(tsih.value, seq.value)
  if (data) {
    detail.value = data
  } else {
...
<template>
  <div class="command-detail">
    <nav class="breadcrumb">
      ...
      <span class="sep">/</span>
      <router-link to="/initiators">Initiators</router-link>
      <span class="sep">/</span>
      <span>Session {{ tsih }}</span>
      <span class="sep">/</span>
